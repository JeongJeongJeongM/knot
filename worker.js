...
function sanitizePrism(prism) {
  ...
  // topic_distribution: object of topic -> number
  ...
  // vocabulary: object of vocab_type -> description
  ...
  // curiosity: object of curiosity traits -> description
...
function sanitizeAnchor(anchor) {
  ...
  // attachment: object of attachment attributes -> description
  ...
  // conflict: object of conflict traits -> description
  ...
  // emotional_availability: object of emotional patterns -> description
  ...
  // growth: object of growth patterns -> description
...
`;
  ...
  // Add PRISM comparison if available
  if (prismA || prismB) {
    prompt += `\n## 공유 세계 (관심사·호기심 비교)\n`;
...
export default {
  async fetch(request, env) {
    ...
    // ──── Future: Payment Gate ────
    // if (path === '/match') {
    //   const paymentVerified = await verifyPayment(body.paymentToken, env);
    //   if (!paymentVerified) return jsonResponse({ error: 'Payment required', code: 'PAYMENT_REQUIRED' }, 402, corsHeaders);
    // }
    ...
    if (path === '/analyze') {
      ...
      try {
        ...
        const prism = sanitizePrism(body.prism);
    ...
    if (path === '/match') {
      ...
      try {
        ...
        const prismA = sanitizePrism(body.prismA);
        const prismB = sanitizePrism(body.prismB);
        const anchorA = sanitizeAnchor(body.anchorA);
        const anchorB = sanitizeAnchor(body.anchorB);
